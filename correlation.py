...
class Correlation:
    ...
    def __init__(self, matrix_file, mfiles,
                 corr_method=None,
                 labels=None,
        ...
        if matrix_file:
            ...
            print("read meth files and merge matrix")
            #self.read_matrix_files(mfiles)
            self.read_matrix_bedfiles(mfiles, context)
    ...
    def read_matrix_bedfiles(self, file_lists, context):
        print("Merge methy files to matrix!\n")
        import pandas as pd
        mylabels = []
 
        merged_df = pd.DataFrame()
        # Use read_csv() to read each file into a DataFrame
        for file_name in file_lists:
            # 读取txt文件，注意设置分隔符为'\t'，第一行为列名
            file_p = os.path.splitext(os.path.basename(file_name))[0]
            mylabels.append(file_p)
            df = pd.read_csv(file_name, sep='\t', header = None, usecols=[0, 1, 2, 3, 4], names=['col1', 'col2', 'col3', 'meth', 'context', 'strand'])
        
            # Use loc[] to filter based on column 5
            if context == 'All':
                filtered_df = df.loc[df['context'] != 'C']
            elif context is not None:
                filtered_df = df.loc[df['context'] == context]

            # Use set_index() to set columns 1-3 as the index
            filtered_df = filtered_df.set_index(['col1', 'col2', 'col3'])
        ...
        self.matrix = np.asarray(merged_df.to_numpy())
    ...
    def read_matrix_files(self, file_lists):
        ...
        for filename in file_lists:
            ...
            mylabels.append(file_name)
            with open(filename, "r") as f:
                first_line = f.readline()
            if first_line.startswith("#"):
                df = pd.read_csv(filename, sep='\t', header=0, names=['ID', file_name])
            else:
                df = pd.read_csv(filename, sep='\t', header=None, names=['ID', file_name])
    ...
    def read_matrix(self, matrix_file):
        # Read the first line and split into header strings
        with open("data.txt", "r") as f:
            header = f.readline().strip()  # read the first line and remove newline character
        header_list = header.split()[1:]  # split the header into a list of strings
        
        # Load data and skip the first column
        data = np.loadtxt("data.txt", skiprows=1, usecols=range(1, len(header_list)+1))
        
        # Delete the first column
        #data = np.delete(data, 0, axis=1)
        
        # Print the header list and data
        ...
        self.labels = list(map(toString, header_list))
...
#        print("mmmm", min_yvalue, max_yvalue)
        # plotly output
        if image_format == 'plotly':
